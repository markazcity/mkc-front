...
{/* OUR PHILOSOPHY */}
<section className="mt-6 bg-white shadow-lg rounded-lg p-4 px-8">
<h2 className="text-2xl font-bold my-2 text-violet-700">JOURNEY SO FAR</h2>
    ...
    leadersData.map((leader)=>{
        ...
        >
            <div className="mr-4 p-6 bg-minigreen-700 bg-opacity-50 hover:bg-opacity-80 rounded-lg">
                <img src={ROOT_URL+"webContents/uploads/"+leader.wc_image} 
                className="fade"
            alt="" /></div>
            <div className="mx-6">{leader.wc_body.substring(0,300)}...
            <br /> <br />
            ...
            className="my-2"
            onClick={()=>{
                window.open(`/admin/customize/editContent?id=${leader.wc_id}`, '_blank').focus();
            }}
            />
            </div>
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 mx-5 lg:mx-32 3xl:mx-64 gap-y-1">
              {pastData.map((item)=>{
                ...
                >
                  ...
                  <span className="text-minigreen-700  text-center">{item.wc_title}</span> <br />
...
export const Journey = ({data, refresh}) => {
    ...
    const [aboutId, setAboutId] = useState("");
      ...
      useEffect(() => {
        if(data!=null){
          let contents = data.filter(item=>item.wc_category==="about_journey");
          if(contents.length>0){
            let aboutContent = contents[0];
