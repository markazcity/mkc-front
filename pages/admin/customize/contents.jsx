...
const Content = () => {
    ...
    fetch(DATA_URL)
      ...
      .then((data) => {
        if (data.status === "success") {
          setData(data.data);
          setEducation(
            data.data.filter((item) => item.wc_category === "education")
          );
          setCommerce(
            data.data.filter((item) => item.wc_category === "commerce")
          );
          setHealth(data.data.filter((item) => item.wc_category === "health"));
          setAgriculture(
            data.data.filter((item) => item.wc_category === "agriculture")
          );
          setResidence(
  ...
  return (
    <AdminLayout title="Contents" label="Dashboard">
      ...
      <br /> <br />
      {/* EDUCATION SECTION */}
      <EditSection dataset={education} title="EDUCATION" />
      {/*HEALTH SECTION */}
      <EditSection dataset={health} title="HEALTH" />
      {/*COMMERCE SECTION */}
      <EditSection dataset={commerce} title="COMMERCE" />
      {/*AGRICULTURE SECTION*/}
      <EditSection dataset={agriculture} title="AGRICULTURE" />
      {/*RESIDENCE SECTION*/}
      <EditSection dataset={residence} title="RESIDENCE" />
    </AdminLayout>
  );
};
...
export const EditSection = ({ dataset, title }) => {
  return (
    <section className="mt-12">
      <h2 className="text-violet-700 font-bold text-3xl">{title}</h2> <br />
      {dataset != null ? (
        dataset.length > 0 ? (
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
            {dataset.map((item) => (
              <div
                className=" bg-gray-100 rounded  shadow hover:shadow-md cursor-pointer"
                key={item.wc_id}
                onClick={() => {
                  window
                    .open(
