              ...
              {blogs.map((blg) => {
                return (
                  <section className="mx-4">
                    <div
                      className="flex bg-gray-100 rounded-lg my-4 px-4 py-2 shadow hover:shadow-md h-52"
                      style={{
                        backgroundImage: `url('${
                          ROOT_URL + "gallery/" + blg.g_fileLink
                        }')`,
        ...
        <div className="py-6 px-4">
          ...
          <button
            ...
            onClick={() => {
              setDeleting(true);
              fetch(
                `https://api.markazcity.in/gallery/delete.php?id=${props.id}&api=${API_KEY}`
              )
                .then((response) => response.json())
                .then((res) => {
                  props.onClose();
